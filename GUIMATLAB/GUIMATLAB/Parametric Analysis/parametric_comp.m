function [net_thrust_array , sfc_array] = parametric_comp(comp_1, comp_2, mach_1 , mach_2)
num = comp_2 - comp_1 + 1;
Temp = zeros(6,1);
Press = zeros(6,1);
...
Temp(1,1) = To;
Press(1,1) = Po;
for comp_ratio = comp_1:comp_2

mach
comp_ratio
PR_c = comp_ratio;
comp_ratio_array(i,j) = PR_c;
Velo_1 = Mo*Velo_sound;
...
figure(1)
plot(comp_ratio_array(i,:), net_thrust_array(i,:) ,'-o')
hold on
title('Net Thrust vs Compressor Pressure Ratio vs Mach Number')
...
figure(3)
plot(comp_ratio_array(i,:) , spec_net_thrust_array(i,:) ,'-o')
hold on
title('Specific Net Thrust vs Compressor Pressure Ratio vs Mach Number')
xlabel('Compression Ratio ')
ylabel('Spec Net Thrust / N.s/kg ')

figure(4)
plot(comp_ratio_array(i,:) , sfc_array(i,:) ,'-o')
hold on
title('Specific Fuel Comsumption vs Compressor Pressure Ratio vs Mach Number')
xlabel('Compression Ratio ')
ylabel('Specific Fuel Comsumption (SFC) / kg/s/N ')

figure(5)
plot(comp_ratio_array(i,:) , air_mass_array(i,:) ,'-o')
hold on
title('Inlet Air Flow vs Compressor Pressure Ratio vs Mach Number')
xlabel('Compression Ratio ')
...
max_sfc = max(sfc_array)
min_sfc = min(sfc_array)
max_netthrust = max(net_thrust_array)
min_netthrust = min(net_thrust_array)

app.MAXEditField.Value = max_sfc;
app.MINEditField.Value = min_sfc;
app.MAXEditField_2.Value = max_netthrust;
app.MINEditField_2.Value = min_netthrust;
