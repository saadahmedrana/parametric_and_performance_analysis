function Iteration_TJ2_Flight_Envelope2
...
% Array of P0 and T0
throttle_array = [.7; 0.8; 0.9; 1; 1.2]; %Throttle setting array will be input by user
mdotf_array = m_fuel*throttle_array; %mdotf we know from above


x0 = .35;
mach_num = zeros(length(mdotf_array),length(MDOTCORR2),length(alt_array));
Thrust = zeros(length(mdotf_array),length(MDOTCORR2),length(alt_array));
...
mach_num(t,r,a) = aaaaa;
 ...
 A4R = 0.3;
 mdot(t,r,a) = rho4(t,r,a)*u4(t,r,a)*A4R;
...
title('mach_num vs. Uninstalled Thrust @ Sea Level');
