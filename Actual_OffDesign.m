...
rpm_array = [0.85 0.87 0.9 0.922 0.964 1.000 1.033 1.063 ];
...
hold ON
plot(z,ratio,'b*')
xlabel('Inlet Mass Flowrate Fraction ')
ylabel('Compressor Pressure Ratio Fraction ')
...
if hit==1 && (count >= 40)
    ...
    PIt(x) = round(pitg1,5);
    CORRECTEDCOMP_RPM(x) = round(Ncorrcg1,3);
...
s_vol = zeros(6,1);
s_entro = zeros(6,1);
offdesign_thrust = zeros(length(PIc),1);
offdesign_sfc = zeros(length(PIc),1);
...
[net_thrust , net_thrust_spec] = thrust_func(Velo_1,Velo_5, m5,Po,p5,A_noz,m_inlet);
thrust1 = net_thrust;
offdesign_thrust(x) = thrust1;

%SFC Calculation

specific_fuel = m_fuel/net_thrust;
sfc1 = specific_fuel;
offdesign_sfc(x) = specific_fuel;
...
figure (1)
plot(CORRECTEDCOMP_RPM(:), offdesign_thrust(:),'-o')
xlabel('Compressor Speed / RPM ')
ylabel('Off-Design Net Thrust / N ')
hold on

figure (2)
plot(CORRECTEDCOMP_RPM(:), offdesign_sfc(:), '-o')
xlabel('Compressor Speed / RPM ')
ylabel('Off-Design Specific Fuel Consumption / (kg/s.N) ')
hold on
