function Iteration_TJ2_Flight_Envelope
...
rpm_array = [0.760 0.820 0.875 0.922 0.964 1.000 1.033 1.063 ];
...
if hit==1 && (count >= 40)
    ...
    PIc(x) = picg1;
    MDOTCORR2(x) = round(mdotcorr2g1,5);
    ETAc(x) =  round(etacg1,5);
    TAUc(x) = round(taucg1,5);
    TT4TT2(x) = round(Tt4Tt2g1,5);
    TAUt(x) = round(tautg1,5);
    ETAt = zeros(length(ETAc),1) + eta_t;
    PIt(x) = round(pitg1,5);
    CORRECTEDCOMP_RPM(x) = Ncorrcg1;
    CORRECTEDTURB_RPM(x) = round(Ncorrtg1,5);
...
alt_array = [10000 ; 12000 ; 14000 ; 16000]; % Alt array will be input by user 

T0 = (15.04-(0.00649*alt_array)) + 273.1;

P0 = (101.29* ((T0(:,:)/288.08).^5.256)) * 1000;
rho00 = P0(:,:)/(R*T0(:,:));
rho0 = rho00(:,1);

% Array of P0 and T0
throttle_array = [.6; 0.7; 0.8; .9; 1]; %Throttle setting array will be input by user
mdotf_array = m_fuel*throttle_array; %mdotf we know from above


x0 = .75;

for r = 4:length(MDOTCORR2)
for a = 1:length(alt_array)
for t =1:length(mdotf_array)
 ...
 %fun = @(mach_num) ((mdotf_array(t)*Qhv*Pstp)/(P0(a)*sqrt(Tstp)*MDOTCORR2(r)*cp_gas*(TT4TT2(r)-TAUc(r))*sqrt(T0(a))))-((((1+((gamma_gas-1)/2)*(mach_num^2)))^4)); 
 %mach_num(t,r,a) = fzero(fun,x0);
 
 syms M0 
eqn = ((mdotf_array(t)*Qhv*Pstp)/(P0(a)*sqrt(Tstp)*MDOTCORR2(r)*cp_gas*(TT4TT2(r)-TAUc(r))*sqrt(T0(a))))-((((1+((gamma_gas-1)/2)*(M0^2)))^4)) == 0;
answer = solve(eqn,M0);
aaaaa = round(answer(2),3);
mach_num(t,r,a) = aaaaa;
    
 u0(t,r,a) = mach_num(t,r,a)*sqrt(gamma_gas*R*T0(a));
 Tt0(t,r,a) = T0(a)*(1+(gamma_gas-1)*(mach_num(t,r,a)^2)/(2)); %[K]
 Pt0(t,r,a) = P0(a)*((1+(gamma_gas-1)*(mach_num(t,r,a)^2)/(2)))^(gamma_gas/(gamma_gas-1));
%  Pt2(t,r,a) = Pt0(t,r,a)*(1-.015*mach_num(t,r,a)^2);
 Pt3(t,r,a) = Pt0(t,r,a)*PIc(r);
 M4R =1;
 P4(t,r,a) = Pt3(t,r,a)/((1+(gamma_gas-1)*(M4R^2)/(2)))^(gamma_gas/(gamma_gas-1));
    
 Tt3(t,r,a) = TAUc(r)*Tt0(t,r,a) ;
 Tt4(t,r,a) = TT4TT2(r)*Tt0(t,r,a);
 if Tt4(t,r,a) > 2500
 Tt4(t,r,a) = nan;
 mach_num(t,r,a) = nan;
 u0(t,r,a)= nan;
 ...
 T4(t,r,a) = Tt4(t,r,a)/(1+(gamma_gas-1)*(M4R^2)/(2));
 u4(t,r,a) = M4R*sqrt(gamma_gas*R*T4(t,r,a));
 rho4(t,r,a) = (P4(t,r,a))/((R*T4(t,r,a)));
 
 
 Tt5(t,r,a) = Tt4(t,r,a)*TAUt(r);
 
 Pt5(t,r,a) = Pt3(t,r,a)*PIt(r);
 M8R = 1;
 P8(t,r,a) = Pt5(t,r,a)/((1+(gamma_gas-1)*(M8R^2)/(2)))^(gamma_gas/(gamma_gas-1));
 T8(t,r,a) = Tt5(t,r,a)/(1+(gamma_gas-1)*(M8R^2)/(2));
 rho8 = P8(t,r,a)/(T8(t,r,a)*R);
 u8(t,r,a) = M8R*sqrt(gamma_gas*R*T8(t,r,a));
 mdot(t,r,a) = rho8*u8(t,r,a)*A_noz;
 if Tt4(t,r,a) > 2500
 Tt5(t,r,a) = nan;
 mdot(t,r,a) = nan;
 Pt5(t,r,a) = nan;
...
vel_diff = u8(t,r,a)-u0(t,r,a)
 
 Thrust(t,r,a) = mdot(t,r,a)*(u8(t,r,a)-u0(t,r,a))+(P8(t,r,a)-P0(a))*A_noz;
 A1R = 0.2;
 mdotref(t,r,a) = rho0(a)*u0(t,r,a)*A1R;
 sprllate(t,r,a) = mdotref(t,r,a)-mdot(t,r,a);
 N(t,r,a) = CORRECTEDCOMP_RPM(r)*sqrt((Tt0(t,r,a))/Tstp);
 if N(t,r,a) > 66000
 Tt4(t,r,a) = nan;
 mach_num(t,r,a) = nan;
 u0(t,r,a)= nan;
...
figure(2)
hold on
plot(mach_num(:,:,1),Thrust(:,:,1),'--k')
% text(mach_num(12,:,1),Thrust(12,:,1),Speedlines,'horizontalalignment','left','verticalalignment','bottom','FontSize',6)
plot(transpose(mach_num(:,:,1)),transpose(Thrust(:,:,1)))
% text(mach_num(:,1,1),Thrust(:,1,1),mdotfnames,'horizontalalignment','left','verticalalignment','top','FontSize',5)
axis([0 2.8 0 1000])
xlabel('mach_num');
ylabel('Uninstalled Thrust [N]');
title('mach_num vs. Uninstalled Thrust @ 10km');

figure(3)
hold on
plot(mach_num(:,:,1),N(:,:,1),'--k')
% text(M0(12,:,1),N(12,:,1),Speedlines,'horizontalalignment','left','verticalalignment','bottom','FontSize',6)
plot(transpose(mach_num(:,:,1)),transpose(N(:,:,1)))
% text(M0(:,1,1),N(:,1,1),mdotfnames,'horizontalalignment','left','verticalalignment','top','FontSize',5)
axis([0 2.8 4.5*10^4 6.6*10^4])
xlabel('M0');
ylabel('Blade Speed (RPM)');
title('M0 vs. Blade Speed @ 10km');

figure(4)
hold on
plot(mach_num(:,:,2),Thrust(:,:,1),'--k')
% text(mach_num(12,:,1),Thrust(12,:,1),Speedlines,'horizontalalignment','left','verticalalignment','bottom','FontSize',6)
plot(transpose(mach_num(:,:,1)),transpose(Thrust(:,:,1)))
% text(mach_num(:,1,1),Thrust(:,1,1),mdotfnames,'horizontalalignment','left','verticalalignment','top','FontSize',5)
axis([0 2.8 0 1000])
xlabel('mach_num');
ylabel('Uninstalled Thrust [N]');
title('mach_num vs. Uninstalled Thrust @ 12km');

figure(5)
hold on
plot(mach_num(:,:,2),N(:,:,1),'--k')
% text(M0(12,:,1),N(12,:,1),Speedlines,'horizontalalignment','left','verticalalignment','bottom','FontSize',6)
plot(transpose(mach_num(:,:,2)),transpose(N(:,:,2)))
% text(M0(:,1,1),N(:,1,1),mdotfnames,'horizontalalignment','left','verticalalignment','top','FontSize',5)
axis([0 2.8 4.5*10^4 6.6*10^4])
xlabel('M0');
ylabel('Blade Speed (RPM)');
title('M0 vs. Blade Speed @ 12km');

figure(6)
hold on
plot(mach_num(:,:,3),Thrust(:,:,3),'--k')
% text(mach_num(12,:,1),Thrust(12,:,1),Speedlines,'horizontalalignment','left','verticalalignment','bottom','FontSize',6)
plot(transpose(mach_num(:,:,3)),transpose(Thrust(:,:,3)))
% text(mach_num(:,1,1),Thrust(:,1,1),mdotfnames,'horizontalalignment','left','verticalalignment','top','FontSize',5)
axis([0 2.8 0 1000])
xlabel('mach_num');
ylabel('Uninstalled Thrust [N]');
title('mach_num vs. Uninstalled Thrust @ 14km');

figure(7)
hold on
plot(M0(:,:,4),N(:,:,4),'--k')
% text(M0(12,:,1),N(12,:,1),Speedlines,'horizontalalignment','left','verticalalignment','bottom','FontSize',6)
plot(transpose(M0(:,:,4)),transpose(N(:,:,4)))
% text(M0(:,1,1),N(:,1,1),mdotfnames,'horizontalalignment','left','verticalalignment','top','FontSize',5)
axis([0 2.8 4.5*10^4 6.6*10^4])
xlabel('M0');
ylabel('Blade Speed (RPM)');
title('M0 vs. Blade Speed @ 14km');
