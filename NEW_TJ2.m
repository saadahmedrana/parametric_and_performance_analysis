...
m_hat = solve((1+(pie_dp-1)*(((N_hat)^(a*b))+2*N_hat*k* log(1-((Xmass-(N_hat^b))/k))))-picg1 , Xmass);
mo_hat = m_hat;
eeta_req = eeta_o * ( 1-(C_eff* (abs((p_hat/(a+delta_a-1)-m_hat)))^c)-D*((abs((m_hat/mo_hat)-1))^d));
etacg1= eeta_req;
...
fprintf('The value of p_ratio is %.3f\n', picg1);
end
