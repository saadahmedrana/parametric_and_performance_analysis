...
B1 = (Tt4Tt2g1*tautg1)*abs(1/(picg1*pib*pitg1))^0.5;
conver_val = (abs(A1-B1));

if count <2 
    Convergence1 = conver_val;
    conver_array(1,count) =  Convergence1;
%     fprintf('The %d value of Convergence is %.3f\n',count, Convergence1);
     if Convergence1 >= tol && (A1>B1)
         fprintf('A > B');
            guess = guess + step;
        elseif Convergence1 >= tol && (A1<B1)
            fprintf('A < B');
            guess = guess - step;
        else
            aaa = 1;
    ...
    temp = conver_array(1,1);
    Convergence1 = conver_val;
    conver_array(1,1) = Convergence1;
    conver_array(1,2) = temp;
%fprintf('The %d value of Convergence is %.3f\n',count, Convergence1);
    diff = conver_array(1,1) - conver_array(1,2);
    if diff > 0
        
        step = step - .005; %We make step smaller
...
count = count+1;
 fprintf('The %d value of supercount and %d Convergence is %.3f\n',supercount,count, Convergence1);
end
